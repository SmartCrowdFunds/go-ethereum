...
import (
	...
	"github.com/Zamolxes-ZMX/go-zmx/contracts/release"
...
func loadConfig(file string, cfg *gzmxConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gzmx.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, gzmxConfig) {
	// Load defaults.
	cfg := gzmxConfig{
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		utils.Fatalf("Failed to register the gzmx release oracle service: %v", err)
	}
	return stack
}
