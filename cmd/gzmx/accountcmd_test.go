...
func TestAccountListEmpty(t *testing.T) {
	gzmx := rungzmx(t, "account", "list")
	gzmx.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gzmx := rungzmx(t, "account", "list", "--datadir", datadir)
	defer gzmx.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gzmx := rungzmx(t, "account", "new", "--lightkdf")
	defer gzmx.ExpectExit()
	gzmx.Expect(`
...
`)
	gzmx.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gzmx := rungzmx(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gzmx.ExpectExit()
	gzmx.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gzmx.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gzmx.StderrText(), m) {
...
// https://github.com/Zamolxes-ZMX/go-zmx/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gzmx.SetTemplateFunc("keypath", func(file string) string {
