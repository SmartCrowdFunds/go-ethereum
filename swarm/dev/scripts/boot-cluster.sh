...
#
# The cluster contains a bootnode, a gscf node and multiple swarm nodes, with
...
# static gscf configuration
GSCF_IP="192.168.33.3"
GSCF_RPC_PORT="8545"
GSCF_RPC_URL="http://${GSCF_IP}:${GSCF_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gscf and
# swarm nodes
start_bootnode() {
...
# start_gscf_node starts a gscf node with --datadir pointing at <base-dir>/gscf
# and a single, unlocked account with password "gscf"
start_gscf_node() {
  local dir="${base_dir}/gscf"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gscf account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GSCF_IP}"
    --rpcport   "${GSCF_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gscf" "${GSCF_IP}" "$(which gscf)" ${args[@]}
}
...
create_account() {
  ...
  gscf --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
