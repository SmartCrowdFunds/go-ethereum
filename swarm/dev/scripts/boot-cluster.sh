...
#
# The cluster contains a bootnode, a gzmx node and multiple swarm nodes, with
...
# static gzmx configuration
gzmx_IP="192.168.33.3"
gzmx_RPC_PORT="8545"
gzmx_RPC_URL="http://${gzmx_IP}:${gzmx_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gzmx and
# swarm nodes
start_bootnode() {
...
# start_gzmx_node starts a gzmx node with --datadir pointing at <base-dir>/gzmx
# and a single, unlocked account with password "gzmx"
start_gzmx_node() {
  local dir="${base_dir}/gzmx"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gzmx account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${gzmx_IP}"
    --rpcport   "${gzmx_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gzmx" "${gzmx_IP}" "$(which gzmx)" ${args[@]}
}
...
create_account() {
  ...
  gzmx --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
